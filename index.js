...
client.once('ready', async () => {
    ...
    const commands = [
        ...
        // 🔒 LỆNH /invite MỚI ĐƯỢC THÊM VÀO ĐÂY (ẨN CHỈ ADMIN DÙNG)
    ...
    try {
        ...
        console.log('Đồng bộ thành công hệ thống lệnh! Lệnh /invite đã được thiết lập.');
    } catch (error) {
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (['invite', 'ticket-dubo', 'mute', 'unmute', 'ban', 'unban', 'role'].includes(interaction.commandName)) {
        ...
        if (interaction.commandName === 'invite') {
            ...
            try {
                // Tạo Embed đúng định dạng giao diện y như ảnh mẫu của bạn
                const inviteEmbed = new EmbedBuilder()
                    .setColor('#2ecc71') // Màu viền xanh lá chuẩn
                    .setAuthor({ 
                        name: 'Dolphin', 
                ...
                return interaction.editReply({ 
                    content: `❌ Thất bại: Không thể gửi Embed vào kênh đó. Vui lòng kiểm tra lại quyền của bot.` 
                });
            }
        }
    ...
    if (interaction.isStringSelectMenu() && ['menu_bloxfruit', 'menu_gag2', 'menu_99night', 'menu_sailor', 'menu_gag', 'menu_forsaken', 'menu_steal_brainrot', 'menu_mm2', 'menu_fisch'].includes(interaction.customId)) {
        ...
        if (interaction.customId === 'menu_gag') { list = gagList; embedColor = '#33cc33'; prefix = "copy_gag"; }
